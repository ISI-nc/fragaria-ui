 ...
 * @author bjonathas
 * 
 *         The parameters are : - a list of journalGroups, - an eventBusRecorder
 *         in order to notifiy all the listeners a JournalElement has been
 *         deleted or has to be edited through a JournalEditEvent - a list of
 *         objects to listen to (optional, they can be set after instanciation
 *         by using the listenTo method) these objects will be able to post
 *         Create/Edit/Cancel event in order to create/edit/remove a
 *         JournalElement from its group (which belongs to the list given as a
 *         parameter).
 * 
 */
...
public class Journal {
	...
	@Subscribe
	public void record(CreateUpdateGroupEvent e) {
	...
	public void onCreateGroup() {
		...
		eventBusRecorder.post(new JournalCreateGroupEvent(grp));
	}

	public void onEditGroup(String gpId) {
		eventBusRecorder.post(new JournalEditGroupEvent(
				getGroupFromGroupsList(gpId)));
	}

	@Subscribe
	public void record(CancelGroupEvent e) {
		if (groups.contains(e.getObject()))
	...
	public void onCancelGroup(String gpId) {
		eventBusRecorder.post(new JournalDeleteGroupEvent(
				getGroupFromGroupsList(gpId)));
	}

	@Subscribe
	public void record(CreateUpdateElementEvent e) {
	...
	public void onEditElement(String eltId, String gpId) {
		eventBusRecorder.post(new JournalEditElementEvent(getElementFromGroup(
				eltId, getGroupFromGroupsList(gpId))));
	}

	@Subscribe
	public void record(CancelElementEvent e) {
	...
	public void onCancelElement(String eltId, String gpId) {
		eventBusRecorder.post(new JournalDeleteElementEvent(
	...
	public JournalElement getElementFromGroup(String eltId, JournalGroup grp) {
		...
		JournalElement elt = from($(e), grp.getElements()).where(
				$(e.getId()).eq(eltId)).uniqueResult($(e));
	...
	public JournalGroup getGroupFromGroupsList(String gpId) {
		...
		JournalGroup grp = from($(g), groups).where($(g.getId()).eq(gpId))
				.uniqueResult($(g));
		return grp;
	}
